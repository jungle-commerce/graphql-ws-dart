...
```dart
import 'package:graphql_ws/graphql_ws.dart';
import 'web_socket_channel_adapter.dart'; // copy into your package
...
`graphql_ws` itself has zero external dependencies — copy the adapter into *your* package (which already depends on `web_socket_channel`).

The same pattern works for any transport: implement `WebSocketAdapter`, expose a `WebSocketConnector` factory, pass it as `connector:`.
...
- **Published `web_socket_channel` adapter package** — currently it's a copy-paste snippet (preserves zero-dep promise for the core).
